...
namespace NfxSample
{
    internal static class EmitExtension
    {
         ...
         )
        {
            ...
            var method = typeBuilder.DefineConstructor(MethodAttributes.Public, typeof(object).GetConstructors().Single().CallingConvention, argsList.ToArray());
            ...
            FieldBuilder[] paramArray)
        {
            var methodinfos = sourceType
                .GetMethods()
                .Where(_method => _method.GetCustomAttribute<JitAopAttribute>() != null)
            ...
            for (var i = 0; i < methodinfos.Length; i++)
            {
                ...
                typeof(AopContext).GetConstructors().FirstOrDefault()
                );
